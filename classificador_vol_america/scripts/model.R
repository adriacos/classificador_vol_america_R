...
train <- function(){
  ...
  1-(table(test_f_90$class_auto_90)/table(test_all$class_auto_90))
  apply(table(test_all[,c("class_pred","class_auto_90")], useNA = "always"),1,function(tb,wg){
    round(tb/wg,2)
  },table(test_all$class_auto_90, useNA = "always"))
  apply(table(test_all[,c("class_pred","class_auto_66")], useNA = "always"),1,function(tb,wg){
    round(tb/wg,2)
  },table(test_all$class_auto_66, useNA = "always"))
  apply(table(test_all[,c("class_pred","class_auto_00")], useNA = "always"),1,function(tb,wg){
    round(tb/wg,2)
...
calc_neighbor_metrics_model_ <- function(v,vect_dt,metrics){
  ...
  for(metric in metrics){
    if(v$neighbors==""){
      ...
      v[,paste("n","mn","dif",metric,sep=""):=mean(vect_dt[id%in%str_split(v$neighbors, ",")[[1]]][[which(colnames(vect_dt)==metric)]],na.rm=T)]  
    }
  }
  return(v)
}
