...
# load("./classificador_vol_america/temp/ids_done.RData")
...
if(length(kms)==0){
  ...
  vect <- readRDS("./classificador_vol_america/vect/set/all.rds")
  km <- "all"
  ori <- "all"
  id <- "all"
  ...
  dir.create(paste("./classificador_vol_america/vect/temp/clumped/",paste(km,"_",ori,sep=""),sep=""),showWarnings=F)
  unlink(paste("./classificador_vol_america/vect/temp/clumped/",paste(km,"_",ori,sep=""),"/",id,".rds",sep=""))
  
  rast <- raster("./classificador_vol_america/rasters/original/original_normalised.tif")
  tryCatch({
  ...
  rm(rast)
  vect <- readRDS(paste("./classificador_vol_america/vect/temp/clumped/",gsub("\\.","-",paste(km,"_",ori,sep="")),"/",id,".rds",sep=""))
  saveRDS(vect,"./classificador_vol_america/vect/clumped/all.rds")
  rm(vect)
  gc()
  ...
  unlink("./classificador_vol_america/temp/kms.RData")
  unlink("./classificador_vol_america/temp/kms_done.RData")
  unlink("./classificador_vol_america/temp/oris_done.RData")
  unlink("./classificador_vol_america/temp/clump_tomovedone.RData")
  library(rstudioapi)
...
oris <- list.dirs(paste("./classificador_vol_america/vect/set/km",km,sep=""),
...
if(length(oris)==0){
  ...
  # stop("new km")
  if(length(kms)>1){
    dirs <- list.dirs(paste("./classificador_vol_america/vect/set/km",km,sep=""))
    ...
    lapply(dirs,function(dir,newkm,oldkm){
      ...
      unlink("./classificador_vol_america/logs/clump_vector_set_quad_ori_kms_dir.txt")
    ...
    saveRDS(vects,"./classificador_vol_america/vect/set/all.rds")
    # unlink(paste("./classificador_vol_america/vect/set/km",km,sep=""))
  }
  ...
  # unlink(paste("./classificador_vol_america/vect/set/",km,sep=""))
  rm(list=ls())
  gc()
...
if(file.exists("./classificador_vol_america/temp/clump_tomovedone.RData")){
  load("./classificador_vol_america/temp/clump_tomovedone.RData")
  if(!is.null(clump_tomovedone)&&clump_tomovedone==T){
    print(paste("clump vector km:",km," ori:",ori," move from done ", Sys.time(),sep=""))
    oridirs <- list.dirs(paste("./classificador_vol_america/vect/set/km",km,sep=""),
                         full.names=T,recursive=F)[sapply(str_split(sub("ori","",
                                                                        list.dirs(paste("./classificador_vol_america/vect/set/km",km,sep=""),
                                                                                  full.names=F,recursive=F)),","),
                                                          function(o,ori){
                                                            ori%in%o
                                                          },ori)]
    lapply(oridirs,function(oridir){
      # print(oridir)
      unlink("./classificador_vol_america/logs/clump_vector_movefromdone.txt")
    ...
    clump_tomovedone <- F
    save(clump_tomovedone,file=("./classificador_vol_america/temp/clump_tomovedone.RData"))
...
clump_vector_do__ <- function(id,files,rast,km,ori,parallel=T,forceddnextract=F,lastkm,lastarealimit){
  ...
  tryCatch({
    clump_vector(vect,rast,quad_id=id,file_id=gsub("\\.","-",paste(km,"_",ori,sep="")),
                 ...
                 arealimit=km*(1+log(km,3)/10)*100,method="closest",log=T,prepare=T,km=km,
                 # method="closest",log=F,prepare=T,km=km,
  ...
  unlink(paste("./classificador_vol_america/vect/temp/clumped/",gsub("\\.","-",paste(km,"_",ori,sep="")),"/",id,".rds",sep=""))
  ...
  lapply(spl_oris,function(spl_ori,km){
      tryCatch({
        ...
        tryCatch({
          if(!dir.exists(paste("./classificador_vol_america/vect/set/km",km,"/ori",ori,sep=""))){
            dir.create(paste("./classificador_vol_america/vect/set/km",km,"/ori",ori,sep=""),showWarnings=F)
          }
          if(!dir.exists(paste("./classificador_vol_america/vect/set/km",km,"/ori",ori,"/done",sep=""))){
            dir.create(paste("./classificador_vol_america/vect/set/km",km,"/ori",ori,"/done",sep=""),showWarnings=F)
          }
          ...
          lapply(spl_ids,function(spl_id,km,ori){
            tryCatch({
                ...
                tryCatch({
                  unlink(paste("./classificador_vol_america/vect/set/km",km,"/ori",ori,"/done/id",id,".rds",sep=""))
                  # dir.create(paste("./classificador_vol_america/vect/set/km",km,"/ori",ori,"/id",id,sep=""),showWarnings=F)
                  saveRDS(spl_id,paste("./classificador_vol_america/vect/set/km",km,"/ori",ori,"/done/id",id,".rds",sep=""))
                  unlink(paste("./classificador_vol_america/vect/set/km",km,"/ori",ori,"/id",id,".rds",sep=""))
                  # saveRDS(spl_id,paste("./classificador_vol_america/vect/set/km",km,"/ori",ori,"/id",id,".rds",sep=""))
                  # unlink(paste("./classificador_vol_america/vect/set/km",km,"/ori",ori,"/id",id,"_.rds",sep=""))
                  print(paste("moved id to done - km:",km," ori:",ori," id:",id,sep=""))
                },error=function(e){
                  print(paste("ERROR km:",km," ori:",ori," id:",id," ",conditionMessage(e),sep=""))
                  print(spl_id)
                  print(paste("./classificador_vol_america/vect/set/km",km,"/ori",ori,"/done/id",id,".rds",sep=""))
...
if(length(ids)>0){
  tryCatch({
    if(length(ids)>1){
      ...
      unlink("./classificador_vol_america/logs/clump_vector_do_launch.txt")
      ...
      res <- parLapplyLB(cl,ids,function(ids,files,rast,km,ori,lastkm,lastarealimit){
        # print(id)
        if(length(ids)>1){
          res <- lapply(ids,clump_vector_do__,files,rast,km,ori,forceddnextract=forceddnextract,lastkm,lastarealimit)
        }else{
          res <- clump_vector_do__(ids[1],files,rast,km,ori,forceddnextract=forceddnextract,lastkm,lastarealimit)
        }
        print(2)
        print(res)
        return(res)
      ...
      lapply(ids,function(id,files,rast,km,ori,lastkm,lastarealimit){
        clump_vector_do__(id,files,rast,km,ori,parallel=F,forceddnextract=forceddnextract,lastkm,lastarealimit)
      },files,rast,km,ori,lastkm,lastarealimit)
    }
  },error = function(e) {
    print("error")
    ...
    rm(list=ls())
    gc()
    log <- read.delim("./classificador_vol_america/logs/clump_vector_do_launch.txt")
    write.table(log,paste("./classificador_vol_america/logs/failed-",Sys.time(),"-clump_vector_do_launch.txt"))
    rm(log)
    unlink("./classificador_vol_america/logs/clump_vector_do_launch.txt")
    stop(paste("ERROR", Sys.time()))
  })
  print(3)
  if(file.exists("./classificador_vol_america/logs/clump_vector_do_launch.txt")){
    log <- read.delim("./classificador_vol_america/logs/clump_vector_do_launch.txt")
    write.table(log,paste("./classificador_vol_america/logs/done-km:",km,"-ori:",ori,"-clump_vector_do_launch.txt"))
    rm(log)
    unlink("./classificador_vol_america/logs/clump_vector_do_launch.txt") 
    unlink(paste("./classificador_vol_america/rasters/temp/",gsub("\\.","-",paste(km,"_",ori,sep="")),"/",sep=""),recursive=T)
  }
  print(4)
}else{
  print(44)
