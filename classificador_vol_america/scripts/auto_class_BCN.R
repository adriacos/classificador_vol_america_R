...
auto_class_BCN <- function(vect){
  source("./classificador_vol_america/scripts/auto_class_BCN.R")
  dir.create("./classificador_vol_america/vect/class_auto",showWarnings=F)
  rast <- raster("C:/Users/acosd/Desktop/CREAF/Mapes/diba/diba_reclass_all.tif")
  ...
  if(!"class_auto_66"%in%colnames(vect_dt)){
    ...
    vect$class_auto_66 <- getclass(vect,rast,0.66)
    vect_dt <- as.data.table(vect)
    rm(vect)
    gc()
  ...
  st_write(st_as_sf(vect_dt),"./classificador_vol_america/vect/class_auto/all.gpkg")
