...
dir <- "./classificador_vol_america/rasters/smoothen/split/"
    ...
    if(length(rs)>1){
      ...
      for(i in 2:length(rs)){
        ...
        if(i==length(rs)){
          ...
          writeRaster(merged, paste("./classificador_vol_america/rasters/smoothen/split/",paste(rep("_",r),collapse=""),newname,sep=""),overwrite=T)
      ...
      writeRaster(rs[[1]], paste("./classificador_vol_america/rasters/smoothen/split/","_",fs,sep=""),overwrite=T)
      unlink(paste("./classificador_vol_america/rasters/smoothen/split/",fs,sep=""))
...
if(length(files_all)>1){
  ...
  writeRaster(raster(paste(dir,files_all[1],sep="")),
              "./classificador_vol_america/rasters/smoothen/merged.tif")
  unlink("./classificador_vol_america/rasters/smoothen/split",recursive=T)
  rm(list=ls())
  gc()
  ...
  if(length(rs)>1){
    ...
    for(i in 2:length(rs)){
      ...
      if(i%%15==0|i==length(rs)){
        # writeRaster(merged, paste("./classificador_vol_america/rasters/pnoa/split/supercells/temp/",newname,sep=""),overwrite=T)
        writeRaster(merged, paste("./classificador_vol_america/rasters/smooothen/split/temp/",newname,sep=""),overwrite=T)
        print("saved") 
        unlink(newlocation)
        unlink(sub(".grd",".gri",newlocation))
        # rm(merged)
    ...
    # writeRaster(rasters[[1]], paste("./classificador_vol_america/rasters/pnoa/split/supercells/temp/",names(rasters[1],".tif"),sep=""),overwrite=T)
    writeRaster(rs[[1]], paste("./classificador_vol_america/rasters/smooothen/split/temp/",names(rs[1]),".tif",sep=""),overwrite=T)
    restartSession(command=source("./classificador_vol_america/scripts/merge_rasters_mosaic.R"))
  }
  rm(list=ls())
  gc()
...
if(length(list.files(paste(dir, "temp/",sep="")))>1){
  ...
  for(i in 2:length(rasters)){
    ...
    # # rm(merged)
    # unlink(paste("./classificador_vol_america/rasters/pnoa/split/supercells/temp/",oldname,sep=""))
    unlink(paste("./classificador_vol_america/rasters/smooothen/split/temp/",oldname,sep=""))
    unlink(r2@file@name)
    print("deleted")
    oldname <- newname
    # plot(merged)
  ...
  file.copy(from=paste("./classificador_vol_america/rasters/smooothen/split/temp/",newname,sep=""),
            to=paste("./classificador_vol_america/rasters/smooothen/","merged.tif",sep=""))
  unlink(newlocation)
  unlink(sub(".grd",".gri",newlocation))
  rm(newname)
  rm(merged)
  ...
  rm(oldlocation)
  rm(oldname)
  unlink("./classificador_vol_america/rasters/smooothen/split/",recursive=T)
}
if(dir.exists("./classificador_vol_america/rasters/smooothen/split/temp")){
  if(length(list.files(paste(dir, "temp/",sep="")))==1){
    if(file.exists(paste("./classificador_vol_america/rasters/smooothen/","merged.tif",sep=""))){
      unlink(paste("./classificador_vol_america/rasters/smooothen/","merged.tif",sep=""))
    }
...
# unlink("./classificador_vol_america/rasters/pnoa/split/",recursive=T)
