...
calc_metrics_ini <- function(){
  ...
  if(!file.exists("./temp/calc_metrics_current.RData")){
    calc_metrics_current <- "ini"
    save(calc_metrics_current,file="./temp/calc_metrics_current.RData")
    rm(calc_metrics_current)
    calc_metrics_ini()
  }else{
    load("./temp/calc_metrics_current.RData")
    stop()
    if(calc_metrics_current=="ini"){
      calc_metrics_set()
    }else if(calc_metrics_current=="set"){
      vect_dt <- readRDS("./vect/metrics/all.rds")
      calc_value(vect_dt)
...
calc_metrics_set <- function(){
  ...
  dir.create("./vect/metrics")
  vect_dt <- readRDS("./vect/clumped/all.rds")
  
  vect_dt <- id <- 1:nrow(vect_dt)
  
  saveRDS(vect_dt,"./vect/metrics/all.rds")
  unlink("./vect/set/all.rds")
...
calc_topographic <- function(vect_dt){
  if(file.exists("./vect/metrics/all_topo.rds")){
    if(!file.exists("./vect/metrics/all.rds")){
      vect_dt <- readRDS("./vect/metrics/all_topo.rds")
      saveRDS(vect_dt,"./vect/metrics/all.rds")
      unlink("./vect/metrics/all_topo.rds")
    }else{
      if(file.info("./vect/metrics/all.rds")$mtime>file.info("./vect/metrics/all_topo.rds")$mtime){
        vect_dt <- readRDS("./vect/metrics/all_topo.rds")
        saveRDS(vect_dt,"./vect/metrics/all.rds")
        unlink("./vect/metrics/all_topo.rds")
      }
  ...
  if(!"elevation"%in%colnames(vect_dt)){
    ...
    gc()
    # library(raster)
    elev <- raster("./maps/elevacions_CAT.tif")
    ...
    saveRDS(vect_dt,"./vect/metrics/all_topo.rds")
    unlink("./vect/metrics/all.rds")
    saveRDS(vect_dt,"./vect/metrics/all.rds")
    unlink("./vect/metrics/all_topo.rds")
  }
...
calc_clima <- function(vect_dt){
  if(file.exists("./vect/metrics/all_climate.rds")){
    if(!file.exists("./vect/metrics/all.rds")){
      vect_dt <- readRDS("./vect/metrics/all_climate.rds")
      saveRDS(vect_dt,"./vect/metrics/all.rds")
      unlink("./vect/metrics/all_climate.rds")
    }else{
      if(file.info("./vect/metrics/all.rds")$mtime>file.info("./vect/metrics/all_climate.rds")$mtime){
        vect_dt <- readRDS("./vect/metrics/all_climate.rds")
        saveRDS(vect_dt,"./vect/metrics/all.rds")
        unlink("./vect/metrics/all_climate.rds")
      }
  ...
  if(!"meantemperature"%in%colnames(vect_dt)){
    ...
    library(raster)
    clima.mean_temp <- raster("./maps/CLIMA_ATLES9120_TMCANUAL.geotiff")
    source("./scripts/clump_vector.R")
    ...
    clima.amp_term <- raster("./maps/CLIMA_ATLES9120_AMPLITUDTERM.geotiff.tif")
    ...
    clima.mean_prec <- raster("./maps/CLIMA_ATLES9120_PPTANUAL.geotiff")
    ...
    clima.mean.temp.wint <- raster("./maps/CLIMA_ATLES9120_TMCGENER.geotiff")
    ...
    saveRDS(vect_dt,"./vect/metrics/all_climate.rds")
    unlink("./vect/metrics/all.rds")
    saveRDS(vect_dt,"./vect/metrics/all.rds")
    unlink("./vect/metrics/all_climate.rds")
  }
...
calc_value <- function(vect_dt){
  if(file.exists("./vect/metrics/all_value.rds")){
    if(!file.exists("./vect/metrics/all.rds")){
      vect_dt <- readRDS("./vect/metrics/all_value.rds")
      saveRDS(vect_dt,"./vect/metrics/all.rds")
      unlink("./vect/metrics/all_value.rds")
    }else{
      if(file.info("./vect/metrics/all.rds")$mtime>file.info("./vect/metrics/all_value.rds")$mtime){
        vect_dt <- readRDS("./vect/metrics/all_value.rds")
        saveRDS(vect_dt,"./vect/metrics/all.rds")
        unlink("./vect/metrics/all_value.rds")
      }
  ...
  if(!"median"%in%colnames(vect_dt)){
    rast <- raster(paste("./rasters/original/original_normalised.tif", sep=""))
    vect <- st_as_sf(vect_dt)
    rm(vect_dt)
    gc()
    ...
    saveRDS(vect_dt,"./vect/metrics/all_value.rds")
    unlink("./vect/metrics/all.rds")
    saveRDS(vect_dt,"./vect/metrics/all.rds")
    unlink("./vect/metrics/all_value.rds")
  }
...
calc_neighbors <- function(vect_dt){
  if(file.exists("./vect/metrics/all_neighbors.rds")){
    if(!file.exists("./vect/metrics/all.rds")){
      vect_dt <- readRDS("./vect/metrics/all_neighbors.rds")
      saveRDS(vect_dt,"./vect/metrics/all.rds")
      unlink("./vect/metrics/all_neighbors.rds")
    }else{
      if(file.info("./vect/metrics/all.rds")$mtime>file.info("./vect/metrics/all_neighbors.rds")$mtime){
        vect_dt <- readRDS("./vect/metrics/all_neighbors.rds")
        saveRDS(vect_dt,"./vect/metrics/all.rds")
        unlink("./vect/metrics/all_neighbors.rds")
      }
 ...
 if(!"neighbors"%in%colnames(vect_dt)){
   ...
   saveRDS(vect_dt,"./vect/metrics/all_neighbors.rds")
   unlink("./vect/metrics/all.rds")
   saveRDS(vect_dt,"./vect/metrics//all.rds")
   unlink("./vect/metrics/all_neighbors.rds")
 } 
...
calc_shape <- function(vect_dt){
  if(file.exists("./vect/metrics/all_shape.rds")){
    if(!file.exists("./vect/metrics/all.rds")){
      vect_dt <- readRDS("./vect/metrics/all_shape.rds")
      saveRDS(vect_dt,"./vect/metrics/all.rds")
      unlink("./vect/metrics/all_shape.rds")
    }else{
      if(file.info("./vect/metrics/all.rds")$mtime>file.info("./vect/metrics/all_shape.rds")$mtime){
        vect_dt <- readRDS("./vect/metrics/all_shape.rds")
        saveRDS(vect_dt,"./vect/metrics/all.rds")
        unlink("./vect/metrics/all_shape.rds")
      }
  ...
  if(!"longness"%in%colnames(vect_dt)){
    ...
    saveRDS(vect_dt,"./vect/metrics/all_shape.rds")
    unlink("./vect/metrics/all.rds")
    saveRDS(vect_dt,"./vect/metrics/all.rds")
    unlink("./vect/metrics/all_shape.rds")
  }
...
calc_texture <- function(vect_dt){
  ...
  if(file.exists("./vect/metrics/all_texture_focal.rds")){
    if(!file.exists("./vect/metrics/all.rds")){
      vect_dt <- readRDS("./vect/metrics/all_texture_focal.rds")
      saveRDS(vect_dt,"./vect/metrics/all.rds")
      unlink("./vect/metrics/all_texture_focal.rds")
    }else{
      if(file.info("./vect/metrics/all.rds")$mtime>file.info("./vect/metrics/all_texture_focal.rds")$mtime){
        vect_dt <- readRDS("./vect/metrics/all_texture_focal.rds")
        saveRDS(vect_dt,"./vect/metrics/all.rds")
        unlink("./vect/metrics/all_texture_focal.rds")
      }
    }
  }else if(file.exists("./vect/metrics/all_texture_tpi.rds")){
    if(!file.exists("./vect/metrics/all.rds")){
      vect_dt <- readRDS("./vect/metrics/all_texture_tpi.rds")
      saveRDS(vect_dt,"./vect/metrics/all.rds")
      unlink("./vect/metrics/all_texture_tpi.rds")
    }else{
      if(file.info("./vect/metrics/all.rds")$mtime>file.info("./vect/metrics/all_texture_tpi.rds")$mtime){
        vect_dt <- readRDS("./vect/metrics/all_texture_tpi.rds")
        saveRDS(vect_dt,"./vect/metrics/all.rds")
        unlink("./vect/metrics/all_texture_tpi.rds")
      }
    }
  }else if(file.exists("./vect/metrics/all_texture.rds")){
    if(!file.exists("./vect/metrics/all.rds")){
      vect_dt <- readRDS("./vect/metrics/all_texture.rds")
      saveRDS(vect_dt,"./vect/metrics/all.rds")
      unlink("./vect/metrics/all_texture.rds")
    }else{
      if(file.info("./vect/metrics/all.rds")$mtime>file.info("./vect/metrics/all_texture.rds")$mtime){
        vect_dt <- readRDS("./vect/metrics/all_texture.rds")
        saveRDS(vect_dt,"./vect/metrics/all.rds")
        unlink("./vect/metrics/all_texture.rds")
      }
  ...
  if(!"sa_1"%in%colnames(vect_dt)){
    ...
    saveRDS(vect_dt,"./vect/metrics/all_texture_focal.rds")
    unlink("./vect/metrics/all.rds")
    saveRDS(vect_dt,"./vect/metrics/all.rds")
    unlink("./vect/metrics/all_texture_focal.rds")
  }
  if(!"tpi_1"%in%colnames(vect_dt)){
    ...
    saveRDS(vect_dt,"./vect/metrics/all_texture_tpi.rds")
    unlink("./vect/metrics/all.rds")
    saveRDS(vect_dt,"./vect/metrics/all.rds")
    unlink("./vect/metrics/all_texture_tpi.rds")
  }
  ...
  if(!"denspl"%in%colnames(vect_dt)){
    ...
    saveRDS(vect_dt,"./vect/metrics/all_texture.rds")
    unlink("./vect/metrics/all.rds")
    saveRDS(vect_dt,"./vect/metrics/all.rds")
    unlink("./vect/metrics/all_texture.rds")
  }
...
calc_neighbor_metrics__ <- function(vect_dt,metrics=NULL,operations=NULL){
  vect_dt <- as.data.frame(vect_dt)
  vect_dt$tmp <- ceiling((1:nrow(vect_dt))/(nrow(vect_dt)/detectCores()))
  vect_spl <- split(vect_dt,vect_dt$tmp)
  unlink("./logs/calc_neighbor_metrics.txt")
  library(parallel)
  source("./scripts/clump_vector.R")
  cl <- create_cluster_clump_(detectCores(),"calc_neighbor_metrics")
  vect_dt <- parLapplyLB(cl,vect_spl,function(vv,vect_dt,metrics,operations){
    do.call(rbind,lapply(1:nrow(vv),function(irow,vv,vect_dt,metrics,operations){
      if(irow%%500==0){
        print(irow)        
      }
      calc_neighbor_metrics_(vv[irow,],vect_dt,metrics,operations)
    },vv,vect_dt,metrics,operations))
  },vect_dt,metrics,operations)
  stopCluster(cl)
  rm(cl)
  unlink("./logs/calc_neighbor_metrics.txt")
  rm(vect_spl)
...
calc_neighbor_metrics <- function(vect_dt){
  if(file.exists("./vect/metrics/all_neighbors_metrics.rds")){
    if(!file.exists("./vect/metrics/all.rds")){
      vect_dt <- readRDS("./vect/metrics/all_neighbors_metrics.rds")
      saveRDS(vect_dt,"./vect/metrics/all.rds")
      unlink("./vect/metrics/all_neighbors_metrics.rds")
    }else{
      if(file.info("./vect/metrics/all.rds")$mtime>file.info("./vect/metrics/all_neighbors_metrics.rds")$mtime){
        vect_dt <- readRDS("./vect/metrics/all_neighbors_metrics.rds")
        saveRDS(vect_dt,"./vect/metrics/all.rds")
        unlink("./vect/metrics/all_neighbors_metrics.rds")
      }
  ...
  if(!"nmndifDN"%in%colnames(vect_dt)){
    vect_dt <- calc_neighbor_metrics__(vect_dt)
    saveRDS(vect_dt,"./vect/metrics/all_neighbors_metrics.rds")
    unlink("./vect/metrics/all.rds")
    saveRDS(vect_dt,"./vect/metrics/all.rds")
    unlink("./vect/metrics/all_neighbors_metrics.rds")
  }
...
calc_neighbor_metrics_ <- function(v,vect_dt,metrics=NULL,operations=NULL){
  # metrics <- c("twi")
  if(is.null(metrics)){
  ...
  if(is.null(operations)){
    ...
    for(metric in metrics){
      if(v$neighbors==""){
        ...
        #   v[,paste("n","sm",metric,sep="")] <- NA
        # }else if(operation=="sd"){
        #   v[,paste("n","sd",metric,sep="")] <- NA
        # }
      }else{
        if(operation=="mean"){
          v[,paste("n","mn",metric,sep="")] <- mean(vect_dt[vect_dt$id %in% str_split(v$neighbors, ",")[[1]],metric],na.rm=T)  
        }
        # else if(operation=="sum"){
        #   v[,paste("n","sm",metric,sep="")] <- sum(vect_dt[vect_dt$id %in% str_split(v$neighbors, ",")[[1]],metric],na.rm=T)  
        # }else if(operation=="sd"){
        #   v[,paste("n","sd",metric,sep="")] <- sd(vect_dt[vect_dt$id %in% str_split(v$neighbors, ",")[[1]],metric],na.rm=T)  
        # }
      }
...
calc_neighbor_metrics_2 <- function(vect_dt){
  if(file.exists("./vect/metrics/all_neighbors_metrics_2.rds")){
    if(!file.exists("./vect/metrics/all.rds")){
      vect_dt <- readRDS("./vect/metrics/all_neighbors_metrics_2.rds")
      saveRDS(vect_dt,"./vect/metrics/all.rds")
      unlink("./vect/metrics/all_neighbors_metrics_2.rds")
    }else{
      if(file.info("./vect/metrics/all.rds")$mtime>file.info("./vect/metrics/all_neighbors_metrics_2.rds")$mtime){
        vect_dt <- readRDS("./vect/metrics/all_neighbors_metrics_2.rds")
        saveRDS(vect_dt,"./vect/metrics/all.rds")
        unlink("./vect/metrics/all_neighbors_metrics_2.rds")
      }
  ...
  if(!"nmndifDN_2"%in%colnames(vect_dt)){
    vect_dt$tmp <- ceiling((1:nrow(vect_dt))/(nrow(vect_dt)/detectCores()))
    vect_spl <- split(vect_dt,vect_dt$tmp)
    unlink("./logs/calc_neighbor_metrics_2.txt")
    library(parallel)
    source("./scripts/clump_vector.R")
    ...
    saveRDS(vect_dt,"./vect/metrics/all_neighbors_metrics_2.rds")
    unlink("./vect/metrics/all.rds")
    saveRDS(vect_dt,"./vect/metrics/all.rds")
    unlink("./vect/metrics/all_neighbors_metrics_2.rds")
  }
...
calc_squaredness <- function(vect){
  ...
  unlink("./logs/calc_squaredness.txt")
  library(parallel)
  source("./scripts/clump_vector.R")
...
calc_longest_line_within <- function(vect_dt){
  ...
  unlink("./logs/calc_longest_line.txt")
  source("./scripts/clump_vector.R")
...
calc_focal_metrics_by_polygons <- function(vect,rast,k=1){
  ...
  library(geodiv)
  unlink("./logs/calc_focal_metrics_by_polygons.txt")
...
calc_TPI_by_polygons <- function(vect,rast,k=1){
  ...
  unlink("./logs/calc_TPI_by_polygons.txt")
