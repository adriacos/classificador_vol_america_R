...
merge_rasters_ <- function(files_split,dir,r,rasterinmemory=NULL){
  ...
  for(fs in files_split){
    ...
    if(length(rs)>1){
      ...
      writeRaster(rs[[1]], paste("./rasters/smoothen2/split/","_",fs,sep=""),overwrite=T)
      unlink(paste("./rasters/smoothen2/split/",fs,sep=""))
      gc()
...
repeat{
  ...
  if(length(files_split)>0){
    ...
    if(length(files_split)>1){
      ...
      if(n.cores==1){
        rasterinmemory <- as.numeric(system("awk '/MemFree/ {print $2}' /proc/meminfo",intern=TRUE))*1024*2
        rasterinmemory <- NULL
        merge_rasters_(files_split,dir,r,rasterinmemory)
        gc()
      }else{
        ...
        unlink("./logs/merge_supercells_rasters_2.txt")
...
if(length(files_all)>1){
  ...
  writeRaster(raster(paste(dir,files_all[1],sep="")),"./rasters/smoothen2/merged.tif")
