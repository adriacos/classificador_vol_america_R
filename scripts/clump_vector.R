...
clump_vector_crop_raster <- function(vect,rast,quad_id,file_id,log=F,outfile="./logs/clump_vector_crop_raster.txt"){
  if(log==T){
    print(paste("crop raster",file_id,"_",quad_id,sep=""))    
    cat(paste("crop raster",file_id,"_",quad_id,sep=""),file=outfile,append=T,sep="\n")    
  }
  dir.create(paste("./rasters/temp/",file_id,"/",sep=""),showWarnings=F)
  unlink(paste("./rasters/temp/",file_id,"/","clump_v_r",
               file_id,"_",quad_id,".gri",sep=""))
  # unlink(paste("./rasters/temp/",file_id,"/","clump_v_r",
  #              file_id,"_",quad_id,".tif",sep=""))
  # if(file.exists(paste("./rasters/temp/",file_id,"/","clump_v_r",
  #                      file_id,"_",quad_id,".gri",sep=""))){
  #   rast <- raster(paste("./rasters/temp/",file_id,"/","clump_v_r",
  #                        file_id,"_",quad_id,".gri",sep=""))
  # }else{
  rast <- crop(rast, st_transform(vect,st_crs(rast)), filename=paste("./rasters/temp/",file_id,"/","clump_v_r",
                                                                     file_id,"_",quad_id,sep=""),overwrite=T)
  # writeRaster(rast,paste("./rasters/temp/",file_id,"/","clump_v_r",
  #                        file_id,"_",quad_id,".tif",sep=""))
  # }
  rast
}
...
clump_vector_remove_repited <- function(vect_dt,quad_id,file_id){
  ...
  if(any(sapply(vect_dt,nrow)>1)){
    ...
    vect_dupl <-rbindlist(lapply(vect_dupl,function(rep,quad_id,file_id){
      ...
      # if(!is.null(tt)&!file.exists(paste("./test/","file_",file_id,"quad_",quad_id,as.data.frame(tt)[1,"id"],".gpkg",sep=""))){
...
extract_metrics_parallel <- function(vect,rast,quad_id,file_id,var,operations=c("mean"),log=F){
  ...
  unlink("./logs/clump_vector_parallelextract.txt")
  rm(vect)
  gc()
  ...
  vect <- do.call(rbind,parLapplyLB(cl,vect_dt,function(vv,quad_id,file_id,rast,log,km_core,var,operations){
    ...
    rm(rast)
    unlink(paste("./rasters/temp/",file_id,"/","clump_v_r",file_id,"_",paste(quad_id,"-",id,sep=""),".gri",sep=""))
    unlink(paste("./rasters/temp/",file_id,"/","clump_v_r",file_id,"_",paste(quad_id,"-",id,sep=""),".grd",sep=""))
    gc()
    vv
...
extract_neighbors_parallel <- function(vect,quad_id,file_id,log=F){
  ...
  rm(unique_ids)
  gc()
  unlink("./logs/clump_vector_neighbours_get.txt")
...
clump_vector_prepare <- function(vect_dt,rast,quad_id,file_id,extract=T,
                                 parallelextract=F,forceddnextract=F,log=F,
                                 checkrepeated=T,outfile="./logs/clump_vector_prepare.txt"){
  if(log==T){
    print(paste(file_id,quad_id,"prepare",Sys.time()))
    cat(paste(file_id,quad_id,"prepare",Sys.time()),file=outfile,append=T,sep="\n")
  }
  time <- Sys.time()
  ...
  if(log==T){
    ...
    cat(paste(file_id,quad_id,"extract",Sys.time()),file=outfile,append=T,sep="\n")
  }
  if(extract==T){
    if(nrow(vect[is.na(vect$DN),])>0|nrow(vect[is.na(vect$sd),])>0|forceddnextract==T){
      ...
      if(parallelextract==T){
        if(log==T){
          ...
          cat(paste("quad:",quad_id," file:",file_id," Parallel extract-",Sys.time()),file=outfile,append=T,sep="\n")
        }
        ...
        # # print("raster croped")
        vect <- clump_vector_extract_values(vect,rast=rast,quad_id=quad_id,file_id=file_id,log=log,operations=c("mean","stdev"),var="value")
        # unlink(paste("./rasters/temp/",file_id,"/","clump_v_r",file_id,"_",quad_id,".gri",sep=""))
        # unlink(paste("./rasters/temp/",file_id,"/","clump_v_r",file_id,"_",quad_id,".grd",sep=""))
      }
      rm(rast)
      gc()
    }
  ...
  if(checkrepeated==T){
    n.row <- nrow(vect)
    if(log==T){
      print(paste(file_id,quad_id,"remove repeated",Sys.time()))
      cat(paste(file_id,quad_id,"remove repeated",Sys.time()),file=outfile,append=T,sep="\n")
    }
    ...
    if(n.row-nrow(vect)!=0){
      ...
      cat(paste("START quad:",quad_id," file:",file_id," ",n.row-nrow(vect)," repeated rows removed",sep=""),file=outfile,append=T,sep="\n")
    }
    gc()
  ...
  if(log==T){
    ...
    cat(paste(file_id,quad_id,"neighbours",Sys.time()),file=outfile,append=T,sep="\n")
  }
  library(nngeo)
  if(parallelextract==T){
  ...
  if(log==T){
    ...
    cat(paste("quad:",quad_id," file:",file_id," Neighbours extracted",sep=""),file=outfile,append=T,sep="\n")
  }
  ...
  if(log==T){
    ...
    cat(paste("quad:",quad_id," file:",file_id," PREPARED",sep=""),file=outfile,append=T,sep="\n")
  }
  
  # if(save_to_disk==T){
  #   saveRDS(vect_dt,paste("./vect/temp/",file_id,"/",quad_id,sep=""))
  #   return(T)
  # }else{
    return(vect_dt) 
  # }
...
clump_vector_simplify <- function(vect_dt,rast,quad_id,file_id,km,
                                  log=T,prepare=T,outfile="./logs/clump_vector_simplify.txt"){
  if(nrow(vect_dt)==0){
    return(T)
  }
  arealimit <- 38*resolution
  
  load("./temp/minres.RData")
  if(log==T){
    print(paste("CLUMP VECTOR",file_id,quad_id,nrow(vect_dt),Sys.time(),sep="-"))
    cat(paste("CLUMP VECTOR",file_id,quad_id,nrow(vect_dt),Sys.time(),sep="-"),file=outfile,append=T,sep="\n")
  }
  dir.create("./vect/temp/clumped/",showWarnings=F)
  dir.create(paste("./vect/temp/clumped/",file_id,"/",sep=""),showWarnings=F)
  load("./temp/resolution.RData")
  
  rast <- clump_vector_crop_raster(st_as_sf(vect_dt),rast,quad_id,file_id,log,outfile=outfile)
  # values(rast) <- 10*values(rast)/255
  vect_dt <- clump_vector_prepare(vect_dt,rast,quad_id,file_id,extract=F,log=log,checkrepeated=F,outfile=outfile)
  ...
  if((max(vect_dt$area)<arealimit)||nrow(vect_dt)==1){
    ...
    saveRDS(vect_dt,paste("./vect/temp/clumped/",file_id,"/",quad_id,".rds",sep=""))
    if(log==T){
      print(paste(file_id,quad_id,
                  "FINISH", Sys.time(),
                  sep=" - "))
      cat(paste(file_id,quad_id,
                  "FINISH", Sys.time(),
                  sep=" - "),file=outfile,append=T,sep="\n")
    }
    return(T)
  }
  if(log==T){
    print(paste("CLUMP VECTOR start algorithm",file_id,quad_id,nrow(vect_dt),Sys.time(),sep="-"))
    cat(paste("CLUMP VECTOR start algorithm",file_id,quad_id,nrow(vect_dt),Sys.time(),sep="-"),file=outfile,append=T,sep="\n")
  }
  gc()
  repeat{
  ...
  print(paste("FINISH",file_id,quad_id,
            ...
            Sys.time(),
            sep=" - "))
    cat(paste("FINISH",file_id,quad_id,
                # as.numeric(system("awk '/MemFree/ {print $2}' /proc/meminfo",intern=TRUE))/1024,
                nrow(vect_dt),
                Sys.time(),
                sep=" - "),file=outfile,append=T,sep="\n")
...
clump_vector <- function(vect_dt,rast,quad_id,file_id,
  ...
  # rast <- raster(paste("./rasters/temp/",file_id,"/","clump_v_r",file_id,"_",quad_id,".tif",sep=""))
  time <- Sys.time()
  ...
  repeat{
    ...
    # vect.min.neighbors <- vect_dt[.(as.numeric(str_split(vect.min$neighbors, ",")[[1]])[!is.na(as.numeric(str_split(vect.min$neighbors, ",")[[1]]))])]
    vect.min.neighbors <- vect_dt[.(as.numeric(str_split(vect.min$neighbors, ",")[[1]]))]
    ...
    # vneighbors <- sapply(str_split(paste(vect.df$neighbors,collapse=","), ",")[[1]],function(nn){
    #   
    # })
    vneighbors <- unique(str_split(paste(vect.df$neighbors,collapse=","), ",")[[1]])
    vneighbors <- vneighbors[vneighbors!=""]
    vneighbors <- vneighbors[!is.na(vneighbors)]
    vneighbors <- vneighbors[!(vneighbors %in% ids)]
    vneighbors <- paste(vneighbors, collapse = ",")
    varea <- sum(vect.df$area)
    # if(simplify==F){
  ...
  #     }  
    vneighbors <- str_split(vect_dt$neighbors, ",")
    vneighbors <- lapply(vneighbors, function(nb,ids){
      nb <- nb[nb!=""]
      nb <- nb[!is.na(nb)]
      nb[nb==ids[2]]<-ids[1] 
      nb
...
create_grid <- function(ext,row,col){
  # if(file.exists(paste("./vect/grids/grid_",row,"x",col,".gpkg",sep=""))){
  #   return(st_read(paste("./vect/grids/grid_",row,"x",col,".gpkg",sep="")))
  # }
  ...
  # st_write(grid, paste("./vect/grids/grid_",row,"x",col,".gpkg",sep=""),overwrite=T)
  grid
}

create_grids <- function(){
  limit <- st_read("./rasters/pnoa/LIMADM_PROVINCIA_BCN_1km.gpkg")
...
clump_vector_ini <- function(){
  ...
  if(!file.exists("./temp/clump_vector_current.RData")){
    ...
    if(clump_vector_current=="ini"){
      ...
      restartSession(command=source("./scripts/clump_vector_big_files_last.R"))
...
get_cuts_from_limit <- function(limit,km){
  library(sf)
  # limit <- read_sf("./vect/grids/limit.gpkg")
  bbox <- st_bbox(limit)
...
clump_vector_set_km_move <- function(km,
  ...
  if(length(files_list)>0){
    dir.create(paste("./vect/set/km",km,sep=""),showWarnings=F)
    ...
    parLapplyLB(cl,files_list,function(file,km,readdir){
      ...
      lapply(spl_oris,function(spl_ori,km,file){
        ...
        lapply(spl_ids,function(spl_id,km,ori,file){
          ...
          unlink(paste("./vect/set/km",km,"/ori",ori,"/id",id,"/",file,sep=""))
          dir.create(paste("./vect/set/km",km,"/ori",ori,"/id",id,sep=""),showWarnings=F)
          # if(nrow(spl_id)>0){
          #   
          # }
          saveRDS(spl_id,paste("./vect/set/km",km,"/ori",ori,"/id",id,"/",file,sep=""))
        },km,ori,file)
      },km,file)
    ...
    rm(cl)
    rm(files_list)
    unlink("./logs/clump_vector_set.txt")
  }
}

clump_vector_set_km_merge <- function(km){
  oridirs <- list.dirs(paste("./vect/set/km",km,"/",sep=""),full.names=TRUE,recursive=F)
  cl <- create_cluster_clump_(detectCores(),"clump_vector_set")
...
clump_vector_set <- function(){
  files_list <- list.files("./vect/vectorised/split/simplified/",
                           pattern="\\.rds$")
  names(files_list) <- sapply(files_list,function(x){
    as.numeric(sub(".rds","",x))
  })  
  # done <- list.files("./vect/vectorised/split/set/",
  # pattern="\\.rds$")
  # files_list <- files_list[!files_list%in%done]
  print(paste("clump vector SET ",length(files_list), " files",sep=""))
  if(length(files_list)>0){
    dir.create("./vect/vectorised/split/set/",showWarnings=F)
    # source("./scripts/clump_vector.R")
    # load("./temp/resolution.RData")
    limit <- st_read("./vect/limit.gpkg")
    if(!file.exists("./temp/kms.RData")){
      load("./temp/resolution.RData")
      ...
      kms <- kms[order(kms)]
      save(kms,file="./temp/kms.RData")
      rm(limit_width)
    }else{
      load("./temp/kms.RData")
    }
    
    # sapply(list.files("./vect/vectorised/split/set"),function(file){
    #   print(file)
    #   readRDS(paste("./vect/vectorised/split/set/",
    #                         file,sep=""))
    #   return(T)
    # })
    
    # cl <- create_cluster_clump_(detectCores(),"clump_vector_set")
    # nrow <- parSapplyLB(cl,list.files("./vect/vectorised/split/set"),function(file){
    #   print(file)
    #   vect <- readRDS(paste("./vect/vectorised/split/set/",
    #                         file,sep=""))
    #   return(nrow(vect))
    # })
    # stopCluster(cl)
    ...
    # gc()
    # 
    # norows <- list.files("./vect/vectorised/split/set")[nrow==0]
    # lapply(norows,function(file){
    #   print(file)
    #   vect <- readRDS(paste("./vect/vectorised/split/set/",
    #                         file,sep=""))
    #   saveRDS(vect,paste("./vect/vectorised/split/norows/",
    #                      file,sep=""))
    #   unlink(paste("./vect/vectorised/split/set/",file,sep=""))
    # })
    
    
    done <- append(list.files("./vect/vectorised/split/set"),
                   list.files("./vect/vectorised/split/norows"))
    files_list <- files_list[!files_list%in%done]
    rm(done)
    rm(nrow)
    gc()
    ...
    unlink("./logs/clump_vector_set.txt")
    cl <- create_cluster_clump_(detectCores(),"clump_vector_set")
    parLapplyLB(cl,files_list,function(file,kms,limit){
      print(file)
      vect <- readRDS(paste("./vect/vectorised/split/simplified/",
                            file,sep=""))
      ...
      nrow3 <- nrow(vect)
      print(paste("id: ",file," nrows: ",nrow1,"-",nrow2,"-",nrow3,sep=""))
      rm(nrow1)
      rm(nrow2)
      rm(nrow3)
      rm(mono)
      ...
      unlink(paste("./vect/vectorised/split/set/",
                   file,sep=""))
      unlink(paste("./vect/vectorised/split/norows/",
                   file,sep=""))
      if(nrow(vect)>0){
        saveRDS(vect,paste("./vect/vectorised/split/set/",
                           file,sep=""))
      }else{
        saveRDS(vect,paste("./vect/vectorised/split/norows/",
                           file,sep=""))
      ...
      # unlink(paste("./vect/vectorised/split/moved_to_set/",
      #              file,sep=""))
      # saveRDS(vect,paste("./vect/vectorised/split/moved_to_set/",
      #                    file,sep=""))
      # 
      # unlink(paste("./vect/vectorised/split/simplified/",
      #              file,sep=""))
    },kms,limit)
    stopCluster(cl)
    rm(cl)
  ...
  rm(files_list)
  # 
  # files <- list.files("./vect/vectorised/split/set/")
  # tt <- unique(unlist(sapply(files,function(file){
  #   unique(readRDS(paste("./vect/vectorised/split/set/",file,sep=""))[,quad_id_3km])
  # })))
  # 
  ...
  # ids_done <- c()
  # save(ids_done, file="./temp/ids_done.RData")
...
clump_vector_do_small_files <- function(){
  ...
  if(file.exists("./vect/clumped/global_clmp.rds")){
    return()
  }
  rast <- raster("./rasters/original/original.tif")
  load("./temp/kms.RData")
  vects <-readRDS("./vect/clumped/global_clmp_temp.rds")
  kms <- kms[-1]
  for (ikm in kms){
    if(file.exists(paste("./vect/clumped/global_clmp_",ikm,"km.rds"))){
      next()
    }
    print(paste(ikm,"km",sep=""))
    # vects <-readRDS("./vect/clumped/global_clmp_temp.rds")
    vects <- clump_vector_do_small_files.launch_km(vects,kms[ikm],rast)
    dir.create("./vect/clumped/kms",showWarnings=F)
    saveRDS(vects, paste("./vect/clumped/kms/global_clmp_",ikm,"km.rds", sep=""))
    saveRDS(vects,"./vect/clumped/global_clmp_temp.rds",overwrite=T)
  }
  saveRDS(vects, "./vect/clumped/global_clmp.rds")
  unlink("./vect/clumped/kms",recursive=T)
  # for(ikm in kms[-1]){
  #   unlink(paste("./vect/clumped/global_clmp_",ikm,"km.rds", sep=""))
  # }
  unlink("./vect/clumped/global_clmp_temp.rds")
  vects
}
...
clump_vector_do_small_files.launch_km_specifyarea <- function(vects_dt,ikm,rast,arealimit,file_id,method=NULL,bigarea=F,simplify=F,parallel=T,log=T,outfile="./logs/clump_vector_do_small_files_launch_km_specifyarea.txt"){
  ...
  unlink(paste("./vect/temp/clumped/",file_id,sep=""),recursive=T)
  unlink(paste("./vect/temp/",file_id,sep=""),recursive=T)
  unlink(paste("./rasters/temp/",file_id,sep=""),recursive=T)
  dir.create(paste("./vect/temp/",file_id,sep=""))
  dir.create(paste("./rasters/temp/",file_id,sep=""))
  for(ori in oris){
    ...
    if(length(unique(vects_ori[[which(colnames(vects_ori)==paste("quad_id_",ikm,"_km_ori_",ori,sep=""))]]))>1){
      ...
      cat(paste(file_id, " ori:",ori," length:",sum(sapply(vects_quad_ori_list,length)),sep=""),file=outfile,append=T,sep="\n")
      
      if(parallel==T){
        unlink("./logs/clump_vector_do_small_files.launch_km_specifyarea.txt")
        cl <- create_cluster_clump_(n.cores,"clump_vector_do_small_files.launch_km_specifyarea")
        parLapplyLB(cl, vects_quad_ori_list, function(vv,rast,quad_id_col,arealimit,method,file_id,ikm,log,outfile){
          # lapply(vects_quad_ori_list, function(vv,rast,quad_id_col,arealimit,method,file_id){
          lapply(vv,function(v,rast,quad_id_col,arealimit,method,file_id,ikm,log,outfile){
            if(is.null(v)){
              return()
            }
            quad_id <- v[[which(colnames(v)==quad_id_col)]][1]
            clump_vector_simplify(v,rast,quad_id,file_id,ikm,log=log,prepare=T,outfile=outfile)
            cat(paste("file:",file_id, " quad:",quad_id," DONE",sep=""),file=outfile,append=T,sep="\n")
            gc()
            return(T)
          },rast,quad_id_col,arealimit,method,file_id,ikm,log,outfile)
          cat(paste("file:",file_id," cluster DONE",sep=""),file=outfile,append=T,sep="\n")
          gc()
          return(T)
        },rast,paste("quad_id_",ikm,"_km_ori_",ori,sep=""),arealimit,method,file_id,ikm,log,outfile)
        stopCluster(cl)
        unlink("./logs/clump_vector_do_small_files.launch_km_specifyarea.txt")
        rm(cl)
        gc()
        cat(paste("file:",file_id," all clusters DONE",sep=""),file=outfile,append=T,sep="\n")
      }else{
        lapply(vects_quad_ori_list, function(vv,rast,quad_id_col,arealimit,method,file_id,ikm,log,outfile){
          # lapply(vects_quad_ori_list, function(vv,rast,quad_id_col,arealimit,method,file_id){
          lapply(vv,function(v,rast,quad_id_col,arealimit,method,file_id,ikm,log,outfile){
            if(is.null(v)){
              return()
      ...
      cat(paste(file_id, " ori:",ori," length:",1,sep=""),file=outfile,append=T,sep="\n")
      ...
      clump_vector_simplify(vects_ori,
                            ...
                            ikm,
                            log=log,
                            prepare=T,outfile=outfile)
    }
    vects_ori <- lapply(paste("./vect/temp/clumped/",file_id,"/",
                              list.files(paste("./vect/temp/clumped/",file_id,"/",sep="")),
                              sep=""),readRDS)
    ...
    if(bigarea==T){
      vects_rest <- readRDS(paste("./vect/temp/clumped//",file_id,"/",
                                  file_id,"_vects_rest.rds",
                                  sep=""))
    }
...
create_cluster_clump_ <- function(n.cores,outfile=NULL){
  ...
  if(Sys.info()['sysname']=="Windows"){
    ...
    cl <- makeCluster(n.cores, outfile=paste("./logs/",outfile,".txt",sep=""))
    ...
    cl <- makeForkCluster(n.cores, outfile=paste("./logs/",outfile,".txt",sep=""))
