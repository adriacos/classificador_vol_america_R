...
smoothen_raster_ini <- function(){
  ...
  if(!file.exists("./temp/smoothen_raster_current.RData")){
    ...
    load("./temp/smoothen_raster_current.RData")
    ...
    if(smoothen_raster_current=="set"){
      ...
      # unlink("./temp/smoothen_raster_grid.RData")
...
smoothen_raster_set <- function(){
  ...
  if(file.exists("./temp/smoothen_raster_grid.RData")){
    ...
    smoothen_raster_grid <- create_grid(limit,round(sqrt(area)/(factor*resolution*3000)),round(sqrt(area)/(factor*resolution*3000)))
    save(smoothen_raster_grid,file="./temp/smoothen_raster_grid.RData")
    rm(area)
    rm(limit)
    rm(resolution)
    unlink("./rasters/original/split",recursive=T)
    unlink("./vect/smoothen_raster_grid",recursive=T)
    dir.create("./vect/smoothen_raster_grid",showWarnings=F)
    lapply(smoothen_raster_grid$id,function(id, smoothen_raster_grid){
      ...
      st_write(buf,paste("./vect/smoothen_raster_grid/",id,".gpkg",sep=""))
      return(T)
    },smoothen_raster_grid)
    gc()
  }
  dir.create("./rasters/original/split",showWarnings=F)
  dir.create("./temp/split",showWarnings=F)
  # ids_done <- sapply(list.files("./rasters/original/split",pattern=".tif$"),function(x){
  #   as.numeric(sub(".tif","",x))
  # })
  # ids_done <- ids_done[order(sapply(list.files("./rasters/original/split",pattern=".tif$",full.names=T),function(x){
  #   file.info(x)$mtime
  # }),decreasing=T)][n.cores:length(ids_done)]
  # # n.cores <- 16
  # smoothen_raster_grid <- smoothen_raster_grid[!smoothen_raster_grid$id%in%ids_done,]
  ids <- sapply(list.files("./vect/smoothen_raster_grid"),function(f){
    gsub(".gpkg","",f)
  })
  ids <- ids[order(as.numeric(ids))]
  print(paste("Smoothen set - grid of ",length(ids), " elements to cut from raster",sep=""))
  # print(paste("Smoothen set - grid of ",nrow(smoothen_raster_grid), " elements to cut from raster",sep=""))
  # n.cores <- detectCores()
  n.cores <- detectCores()
  unlink("./logs/smoothen_raster_set.txt")
  ...
  parLapplyLB(cl,ids,function(id, smoothen_raster_grid, rast,rasterinmemory){
    print(id)
    ...
    mask <- mask(crop,buf,filename=paste("./rasters/original/split/",id,".tif",sep=""))
    unlink(crop@file@name)
    # crop <- mask(crop(rast,buf),buf)
    # writeRaster(crop,paste("./rasters/smoothen2/split/",id,".tif",sep=""))
    unlink(paste("./vect/smoothen_raster_grid/",id,".gpkg",sep=""))
    rm(crop)
    rm(mask)
    gc()
    ...
    # # writeRaster(crop,paste("./rasters/original/split/",id,".tif",sep=""))
    # rm(mask)
    # gc()
  },smoothen_raster_grid,rast,rasterinmemory)
  stopCluster(cl)
  rm(cl)
  gc()
  unlink("./logs/smoothen_raster_set.txt")
  ...
  if(nrow(smoothen_raster_grid)>0){
    restartSession(command=source("./scripts/smoothen_raster.R"))
  }
...
#   load("./temp/max_val.RData")
#   load("./temp/smoothen_raster_grids.RData")
...
smoothen_raster_supercells_ <- function(rast,max_val,smoothen_raster_grid,pretty=T){
  ...
  if(ceiling(9*area/500)<3){
    writeRaster(raster(rast),paste("./rasters/smoothen/split/",id,".tif",sep=""))
    unlink(paste("./rasters/original/split/",id,".tif",sep=""))
    return()
  }
  ...
  writeRaster(rast_sc,paste("./rasters/smoothen/split/",id,".tif",sep=""))
  unlink(paste("./rasters/original/split/",id,".tif",sep=""))
  # rast_sc
  return(TRUE)
}
...
smoothen_raster_dp <- function(id, threshold=0.15){
  ...
  rast <- raster(paste("./rasters/", id, ".tif", sep=""))
  ...
  rast <- rast*10
  res <- writeRaster(rast,paste("./rasters/smoothen/",id,"_smth_dp.tif", sep=""), overwrite=TRUE)
...
# C:/Users/acosd/Desktop/CREAF/Proves/classificador_vol_america_R/rasters
# 
